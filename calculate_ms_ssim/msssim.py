...
tf.flags.DEFINE_string('data_path', None, 'Path to directory of PNG image.')
tf.flags.DEFINE_string('outf', None, 'Path to output directory.')
tf.flags.DEFINE_string('log_name', None, 'Path to log file, such as "log.txt".')
...
def main(_):
	...
	# obtain the filename of images
	image_list = os.listdir(FLAGS.data_path)
	...
	with tf.Session() as sess:
		for i in range(length):
			# img1_str = os.path.join(FLAGS.data_path, image_list[i*2])
			# img2_str = os.path.join(FLAGS.data_path, image_list[i*2+1])

			with tf.gfile.FastGFile(os.path.join(FLAGS.data_path, image_list[i*2])) as image_file:
				 img1_str = image_file.read()
			with tf.gfile.FastGFile(os.path.join(FLAGS.data_path, image_list[i*2+1])) as image_file:
				 img2_str = image_file.read()
			...
			ms_ssmi_value = MultiScaleSSIM(img1, img2, max_val=255)
			if math.isnan(ms_ssmi_value):
				print("[{}]:{} is removed".format(i, ms_ssmi_value))
			else:
				ms_ssmi_value_list.append(ms_ssmi_value)

			if i%50==0:
				print("[{}]:{}".format(i, ms_ssmi_value))
				print("current len of list: {}".format(len(ms_ssmi_value_list)))
	...
	average = sum(ms_ssmi_value_list)/len(ms_ssmi_value_list)
	print("Average: {}".format(average))

	with open(os.path.join(FLAGS.outf, FLAGS.log_name), 'w') as f:
		f.write("Average MS-SSIM of {} samples: {}".format(len(image_list) ,average))
